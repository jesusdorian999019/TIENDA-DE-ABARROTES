...
class InventoryTab:
    ...
    def _create_ui(self):
        ...
        self.tree = ttk.Treeview(
            table_frame,
            columns=('Código', 'Nombre', 'Categoría', 'Proveedor', 'P. Compra', 'P. Venta', 'Stock', 'Stock Min.'),
            height=15,
        ...
        self.tree.column('Categoría', anchor=tk.W, width=120)
        self.tree.column('Proveedor', anchor=tk.W, width=120)
        self.tree.column('P. Compra', anchor=tk.CENTER, width=100)
        self.tree.column('P. Venta', anchor=tk.CENTER, width=100)
        self.tree.column('Stock', anchor=tk.CENTER, width=80)
        self.tree.column('Stock Min.', anchor=tk.CENTER, width=80)
    ...
    def _add_product(self):
        ...
        # Precio de compra
        ttk.Label(main_frame, text="Precio Compra:").grid(row=4, column=0, sticky=tk.W, pady=5)
        purchase_var = tk.StringVar()
        ttk.Entry(main_frame, textvariable=purchase_var, width=40).grid(row=4, column=1, pady=5)
        fields['purchase_price'] = purchase_var
        
        # Precio de venta
        ttk.Label(main_frame, text="Precio Venta:").grid(row=5, column=0, sticky=tk.W, pady=5)
        sale_var = tk.StringVar()
        ttk.Entry(main_frame, textvariable=sale_var, width=40).grid(row=5, column=1, pady=5)
        fields['sale_price'] = sale_var
        
        # Stock
        ttk.Label(main_frame, text="Stock:").grid(row=6, column=0, sticky=tk.W, pady=5)
        stock_var = tk.StringVar()
        ttk.Entry(main_frame, textvariable=stock_var, width=40).grid(row=6, column=1, pady=5)
        fields['stock'] = stock_var
        
        # Stock mínimo
        ttk.Label(main_frame, text="Stock Mínimo:").grid(row=7, column=0, sticky=tk.W, pady=5)
        min_stock_var = tk.StringVar()
        ttk.Entry(main_frame, textvariable=min_stock_var, width=40).grid(row=7, column=1, pady=5)
        fields['min_stock'] = min_stock_var
        
        # Botones
        button_frame = ttk.Frame(main_frame)
        button_frame.grid(row=8, column=0, columnspan=2, pady=20, sticky=tk.E)
