...
class ReportsService:
    ...
    def get_inventory_report(self) -> Dict[str, Any]:
        ...
        total_capital = sum(p.stock * p.purchase_price for p in products)
    ...
    def get_sales_report_by_date(self, date_str: str = None) -> Dict[str, Any]:
        ...
        # Calcular capital recuperado y ganancia (v1.0.2)
        # Capital recuperado = suma de (cantidad * precio_compra)
        total_capital_recovered = sum(s.quantity * s.purchase_price for s in sales)
        # Ganancia = ingresos - capital recuperado
        ...
        return {
            ...
            'capital_recovered': total_capital_recovered,
            'profit': total_profit,
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        today_capital_recovered = sum(s.quantity * s.purchase_price for s in today_sales)
        today_profit = today_revenue - today_capital_recovered

        # Resumen general
        all_sales = self.data_manager.get_all_sales()
        total_revenue = sum(s.total for s in all_sales)
        total_capital_recovered = sum(s.quantity * s.purchase_price for s in all_sales)
        total_profit = total_revenue - total_capital_recovered
        ...
        total_capital_invested = sum(p.purchase_price * p.stock for p in products)

        return {
            'today_sales_count': len(today_sales),
            'today_revenue': today_revenue,
            'today_capital_recovered': today_capital_recovered,
